...
$parentModule = 'CloudPC'
if (-not $ModuleName) { $ModuleName = 'CloudPC.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\CloudPC'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\CloudPC.psd1"
$prereqHash = @{
