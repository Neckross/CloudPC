...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'CloudPC.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'CloudPC.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'CloudPC.Client.ProtectedToken'
